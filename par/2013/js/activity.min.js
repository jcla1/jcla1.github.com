...
function timeInterval(b){function e(b,c,d){return 0===c?b:(b?b+" ":b)+parseInt(c)+d}var c="",g=parseInt(b/86400),d,f;b%=86400;d=parseInt(b/3600);b%=3600;f=parseInt(b/60);b%=60;c=e(c,g,"d");c=e(c,d,"h");c=e(c,f,"m");return e(c,b,"s")}function round6(b,e){var c=10*b.getUTCHours()+b.getUTCMinutes()/6;return!0==e&&0==c?240:c}
d3.csv("data/activity_log.csv",function(b){function e(a){return 24==a||0==a?"12a":12>a?a+"a":12===a?a+"p":a%12+"p"}function c(a){function b(a){return d-(a.y+a.durationHeight)}m=n.selectAll("rect");a=m.data(a);a.transition().delay(10).duration(900).attr("x",function(a){return a.x}).transition().delay(900).duration(900).attr("y",b).attr("height",function(a){return a.height});a.enter().append("rect").attr("class",function(a,b){return"activity"}).attr("width",1).style("fill","#2CA02C").attr("x",function(a){return a.x}).transition().delay(900).duration(900).attr("y",
b).attr("height",function(a){return a.height});a.exit().transition().delay(10).duration(900).attr("x",function(a){return g+a.x}).attr("height",0).remove()}var g=813,d=490,f=d3.scale.linear().range([d,0]).domain([0,240]),q=d3.scale.linear().range([0,d]).domain([0,86400]),l=d3.time.scale().range([0,g]).domain([new Date(2013,0,1),new Date(2013,11,31)]).nice(),r=d3.time.format("%y%m%d"),h={};b.forEach(function(a){a.duration=+a.duration;a.start_ts=+a.start_ts;a.end_ts=+a.end_ts;var b=new Date(1E3*a.start_ts),
c=new Date(1E3*a.end_ts);a.day=r(b);c=round6(c,!0);a.height=d-f(c-round6(b));a.durationBlock=a.end_ts-a.start_ts;a.durationHeight=q(a.durationBlock);h[a.program]=(h[a.program]||0)+a.duration;a.y=f(c);a.x=l(d3.time.day(b))});var k=d3.nest().key(function(a){return a.program}).map(b);b=d3.keys(k).sort(function(a,b){return h[a]<h[b]?1:-1});d3.select("#computer_activity_programs").selectAll("a").data(b).enter().append("a").attr("class",function(a,b){return"activity_btn"+(0==b?" active":"")}).on("click",
function(a,b){d3.select("#computer_activity_programs").selectAll("a").classed("active",function(a,c){return b==c});c(k[a])}).append("img").attr("width",16).attr("height",16).attr("src",function(a){return icons[a]}).attr("alt",function(a){return a}).attr("title",function(a){return a+" - "+timeInterval(h[a])});b=d3.select("#activity").append("svg").attr("width",860).attr("height",530).append("g").attr("transform","translate(40,10)");var s=d3.svg.axis().scale(l).orient("bottom").tickFormat(d3.time.format("%b"));
b.append("g").attr("class","x axis").attr("transform","translate(0,"+d+")").call(s);b.selectAll(".x.axis text").style("text-anchor","middle").attr("x",36).attr("y",7);var p=b.append("g").attr("class","bands");p.selectAll(".band").data(d3.range(24)).enter().append("rect").attr("class",function(a,b){return 0==b%2?"band band_even":"band band_odd"}).attr("x",-40).attr("width",g+40).attr("y",function(a){return f(10*a)-d/24}).attr("height",d/24);d3.format("d");(function(){var a=p.selectAll("text").data(d3.range(24).reverse());
a.transition().text(function(a){return e(a)});a.enter().append("text").attr("x",-12).attr("y",function(a){return f(10*(23-a))-d/24/2}).attr("dy",".5em").style("text-anchor","end").transition().text(function(a){return e(a)});a.exit().remove()})();var n=b.append("g"),m=n.selectAll("rect");c(k["All Programs"])});